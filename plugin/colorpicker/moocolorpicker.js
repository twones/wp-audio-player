var MooColorPicker = new Class({
	...
	_buildColorPicker:function()
	{
		...
		if(this.options.panelMode)
		{
			...
			this.colorpicker.makeDraggable({
				handle:header,
				onDrag:this._updatePanelCoords.bind(this)
			});
		}
		...
		this.colorpicker.injectInside(this.container);
	...
	show:function()
	{
		if(!this.options.panelMode) return;
		this.colorpicker.setStyle("display", "block");
		this.state = "open";
	},
